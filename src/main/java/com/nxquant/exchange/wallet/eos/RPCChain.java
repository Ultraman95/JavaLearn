...
public class RPCChain implements Chain {
    ...
    public List<String> getCurencyBalance(String account, String code, String symbol) throws ChainException {

        List<String> currencyBalance = new  ArrayList<String>();
        try {
            URL getBlockURL = new URL(chainURL, "/v1/chain/get_currency_balance");
            ...
            if (response.response != null) {
                currencyBalance =  _objectMapper.readValue(response.response, List.class);
            } else {
                ErrorResponse errorResponse = _objectMapper.readValue(response.error.getEntity().getContent(), ErrorResponse.class);
                throw new ChainException(errorResponse.message, errorResponse);
            }
