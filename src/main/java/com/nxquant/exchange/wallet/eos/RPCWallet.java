...
import com.nxquant.exchange.wallet.eos.exception.EOSException;
import com.nxquant.exchange.wallet.eos.exception.WalletException;
import com.nxquant.exchange.wallet.eos.messages.ErrorResponse;
...
public class RPCWallet implements Wallet {
    ...
    public String createKey(String walletName) throws WalletException {
        ...
        try {
            URL getInfoURL = new URL(walletURL, "/v1/wallet/create_key");

            //String walletName = "\"" + name + "\"";

            String request = "[\"" + walletName + "\",\"K1\"]";

            EOSRPCAdapter.EOSRPCResponse response = rpcAdapter.postRequest(getInfoURL, request);


            if (response.response != null) {
                newKeyWif = response.response.replaceAll("\"", "");
            } else {
